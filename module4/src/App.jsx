...
const SLIDES = [
  ...
  { id: 's4', type: 'codeblock', title: 'The Build Package That Comes Out',
    intro: 'The Decompose run from the previous slide produces this build package. Header metadata, then a wave plan listing every component with type tag and one-line purpose. Continues into interface contracts and a dependency graph (omitted here for length).',
...
# Components: 6 | Parallel: 6
- UserModel       [data_model] — User entity and persistence
- TeamModel       [data_model] — Team entity and persistence
- TaskModel       [data_model] — Task entity and persistence
- CommentModel    [data_model] — Comment entity and persistence
- DatabaseConfig  [tech_stack] — Database connection and configuration
- DeploymentConfig[deployment] — Infrastructure, CI/CD, environment configuration

## Wave 1: Foundation (Models + Config)
# Components: 2 | Parallel: 2 | Dependencies: Wave 0
- CacheService [tech_stack, performance] — Caching layer configuration and operations
- AuthService  [auth]                    — Authentication and token management

## Wave 2: Services
# Components: 7 | Parallel: 7 | Dependencies: Wave 1
- AuthMiddleware    [auth] — Request authentication and authorization middleware
- TeamHandler       [api]  — REST endpoints for team resource
- TaskListHandler   [api]  — GET /api/teams/:teamId/tasks
- TaskCreateHandler [api]  — POST /api/teams/:teamId/tasks
- TaskUpdateHandler [api]  — PATCH /api/tasks/:id
- TaskHandler       [api]  — REST endpoints for task resource
- CommentHandler    [api]  — POST /api/tasks/:id/comments

## Interface Contracts (excerpt)
- IAuthService:
...
- team-handler    → auth-service (consumes IAuthService)
- task-*-handler  → auth-service (consumes IAuthService)
- *-handler       → database-config (config dependency)` },
