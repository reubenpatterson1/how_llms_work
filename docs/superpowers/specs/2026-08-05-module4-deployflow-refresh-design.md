...
- **New slide `s8b`**, inserted immediately after `s8`: one combined slide with two screenshots — the Deploy page's "✓ Live at" success state, and the actual live deployed app page in a browser. Requires a small additive change to the image-slide renderer in `App.jsx` to support multiple images on one slide (e.g. `images: [img1, img2]` alongside the existing single-`image` case) — no other slide's rendering changes.
