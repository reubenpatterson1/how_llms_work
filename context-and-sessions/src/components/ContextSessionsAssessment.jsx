...
function IntroScreen({ onStart, name, setName }) {
  return (
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '60px 40px', textAlign: 'center' }}>
      ...
      <h2 style={{ color: C.text, fontSize: 28, marginBottom: 8 }}>Context & Sessions Assessment</h2>
      <p style={{ color: C.accent, fontSize: 16, marginBottom: 24 }}>Attention, Prompts, Governance</p>
      <p style={{ color: C.textDim, fontSize: 14, marginBottom: 32, lineHeight: 1.6 }}>
        15 questions across 3 sections testing your grasp of lost-in-the-middle dynamics,
        the balance of prompt and context engineering, and session hygiene practices.
...
async function downloadCertPDF({ title, subtitle, name, body, fields, watermark = 'LLM ENGINEERING COURSE' }) {
  ...
  if (watermark) { /* watermark param retained for API parity */ }
...
function Certificate({ name, score, total, sectionScores }) {
  ...
  const [id] = useState(() => `CTXS-${hashCode(name + score)}-${Date.now().toString(36).toUpperCase()}`)

  // Persist completion once per mount, not during render.
  useEffect(() => {
    if (!passed) return
    try {
    ...
    <div style={{ maxWidth: 700, margin: '0 auto', padding: '40px 30px' }}>
      <div style={{
        ...
        <p style={{ color: C.accent, fontSize: 14, marginBottom: 24 }}>
          Context & Sessions — Attention, Prompts, Governance
        </p>
        ...
        <p style={{ color: C.textDim, fontSize: 11 }}>
          Context & Sessions — {new Date().toLocaleDateString()}
        </p>
      </div>
      ...
      <div style={{ textAlign: 'center', marginTop: 16 }}>
        <button onClick={() => downloadCertPDF({
            ...
            subtitle: 'Context & Sessions — Attention, Prompts, Governance',
            name,
            body: 'has demonstrated engineering-level comprehension of lost-in-the-middle dynamics, the balance of prompt and context engineering, session hygiene practices, and context governance for large language models.',
            fields: [
